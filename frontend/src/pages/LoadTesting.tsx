...
const LoadTesting = () => {
  ...
  const handleStart = async () => {
    ...
    try {
      // Interpreting requestRate as "Per Thread"
      // If user sets 10 req/sec and 3 threads, total throughput = 30 req/sec
      const perThreadRate = config.requestRate;
      const totalRequestsPerThread = Math.ceil(perThreadRate * config.duration);
      
      // Call real backend API
      const response = await rateLimiterApi.runLoadTest({
        concurrentThreads: config.concurrency,
        requestsPerThread: totalRequestsPerThread,
        durationSeconds: config.duration,
        tokensPerRequest: config.tokensPerRequest,
        // Delay is 1000ms / per-thread-rate
        delayBetweenRequestsMs: config.pattern === 'spike' ? 0 : 1000 / perThreadRate,
