...
export const ScheduleForm = ({ schedule, onClose, onSuccess }: ScheduleFormProps) => {
  const [formData, setFormData] = useState<ScheduleRequest>({
    ...
    limits: {
      capacity: schedule?.activeLimits?.capacity ?? 100,
      refillRate: schedule?.activeLimits?.refillRate ?? 10,
      algorithm: schedule?.activeLimits?.algorithm ?? 'TOKEN_BUCKET',
    },
    fallbackLimits: {
      capacity: 10,
      refillRate: 2,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.type === 'RECURRING') {
      (submitData as any).startTime = null;
      (submitData as any).endTime = null;
    } else {
      (submitData as any).cronExpression = null;
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              ...
              {(formData.limits.algorithm === 'FIXED_WINDOW' || formData.limits.algorithm === 'SLIDING_WINDOW') ? (
                <div className="lg:col-span-1">
                  <Label htmlFor="requestsPerSec">Requests per Second</Label>
                  <Input
                    id="requestsPerSec"
                    type="number"
                    ...
                    onChange={(e) => {
                      ...
                      handleLimitsChange('capacity', val);
                      handleLimitsChange('refillRate', val); // Keep in sync for window logic
                    }}
                    required
                  />
                </div>
